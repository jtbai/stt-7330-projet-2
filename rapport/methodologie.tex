...
Pour le modèle SVM, les hyperparamètres utilisés sont cost et epsilon ($C$ et $ICITTE$ dans la notation utilisée dans le cours).\\
...
Alors que le modèle SVM maximise la marge entre deux classes, le paramètre $\xi$ contrôle l'erreur relative à la marge que le classifieur peut 


Nous avons considéré les valeurs de 0.1,0.01 et.001. 
...
Les modèles par ensemble sont inspirés du proverbe \guillemotleft . Par exemple, si il y a trois classifieurs indépendants qui ont une exactitude de 0.65 et qu'on utilise un modèle de vote majoritaire, la probabilité que la majorité se trompe est 
